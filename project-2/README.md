...
* `Creator Info`:
*   Name: Jesse Baxter
*   Contact: jbaxter@uoregon.edu

## Installation
* In order to run the contents of the repo install docker and run `docker build -t NAME`from 
* within project-2 directory. After the docker image has been build run 
* `docker run -p YOURPORT:5000 NAME` to allow for portforwarding out of the image to your
* local machine.

## Functionality
* A simple server written using Flask and utilizing a docker container to ensure portability.
* Takes in requests in `GET` requests of the form of `localhost:port/query` and searches to 
* see if there exists a file containing query in the `/pages` directory. If an illegal 
* character such as `..` or `~` is used in the query then a response of `403 forbidden` will 
* be sent. If the page isnt found than a response of `404 not found` is sent. Otherwise a 
* response of `200 OK` and the page contents are returned.

* If the query isn't a `GET` request then an error is thrown.
