...
import os        # Used for finding file
...
def respond(sock, docroot):
    ...
    Any valid GET request is answered with the appropriate requested page or an error page.
    """
    sent = 0
    ...
    if len(parts) > 1 and parts[0] == "GET":
        ...
        if "~" in parts[1] or ".." in parts[1]:
          ...
          transmit("Forbidden character, '..' or '~' used in search.", sock)

        #Determine if the file is present or not
        elif os.path.isfile(f"{docroot}/{parts[1]}"):
          transmit(STATUS_OK, sock)
          with open(f"{docroot}/{parts[1]}") as response:
           transmit(response.read(), sock)
  
        #If query finds nothing
        else: 
            transmit(STATUS_NOT_FOUND, sock)
            transmit("Error, page not found.", sock)
